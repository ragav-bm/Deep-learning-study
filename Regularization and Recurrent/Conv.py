...
class Conv(BaseLayer):
    def __init__(self,stride_shape,convolution_shape,num_kernels):
        ...
        if len(self.convolution_shape) ==3:
            self.weights = np.random.uniform(0, 1.0, size=(self.num_kernels,self.convolution_shape[0] , self.convolution_shape[1],self.convolution_shape[2]))
            self.bias = np.random.uniform(0, 1.0, size=(self.num_kernels,))
        else:
            self.weights = np.random.uniform(0, 1.0, size=(self.num_kernels,self.convolution_shape[0] , self.convolution_shape[1]))
            self.bias = np.random.uniform(0, 1.0, size=(self.num_kernels,))
    ...
    def forward(self, input_tensor):
        ...
        if(len(input_tensor.shape) ==3):
            ...
            input_padded = np.pad(input_tensor,pad_width=((0,0),(0,0),(p_2,p_1)), mode ='constant', constant_values = 0)
            ...
            for batch in range(b):
                for kernel in range(self.num_kernels):
                    for channel in range(c):
                        ...
                        output_tensor[batch,kernel] = output_tensor[batch,kernel] + correlate(input_padded[batch,channel],self.weights[kernel,channel],mode='valid')[::self.stride_shape[0]]
            ...
            input_padded = np.pad(input_tensor,pad_width=((0,0),(0,0),(p_h_2,p_h_1),(p_w_2,p_w_1)),mode='constant',constant_values =0)


            for batch in range(b):
                for kernel in range(self.num_kernels):
                    for channel in range(c):
                        output_tensor[batch,kernel] = output_tensor[batch,kernel] + correlate2d(input_padded[batch,channel],self.weights[kernel,channel],mode='valid')[::self.stride_shape[0], :: self.stride_shape[1]]
        ...
        for item in range(batch_size):
            for c in range(num_channels):
                if len(self.stride_shape) == 1:
                    error_t[:, :, ::self.stride_shape[0]] = error_tensor[item]
                else:
                    error_t[:, :, ::self.stride_shape[0], ::self.stride_shape[1]] = error_tensor[item]

                t_output = convolve(error_t[item], weights[c], 'same')
                t_output = t_output[t_output.shape[0] // 2]
                gradient_input[item, c] = t_output
            ...
            input_pad = np.pad(self.input_tensor[item], padding_width, mode='constant', constant_values=0)
            
            for ker in range(self.num_kernels):
                for c in range(num_channels):
                    tempory_gradient[ker, c] = correlate(input_pad[c], error_t[item][ker], 'valid')
